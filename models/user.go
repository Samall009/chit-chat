...
func (user *User) Session() (session Session, err error) {
	...
	// 预处理语句
	SQL := "select id, uuid, email, user_id, created_at from sessions where user_id = ?"
	// 查询单条数据
	err = Db.QueryRow(SQL, user.Id).Scan(&session.Id, &session.Uuid, &session.Email, &session.UserId, &session.CreatedAt)
	if err != nil {
		return session, err
	}
